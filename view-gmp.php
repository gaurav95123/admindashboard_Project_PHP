...
<table class="table table-hover text-center">
  <thead class="table-light">
    <h1 class="p-3">Salsa User Details</h1>
    <tr>
      <th>ID</th>
      <th>Date</th>
      <th>Auditee</th>
...
-- INNER JOIN crud_table ct ON gmp1.id = ct.cid";

// -- INNER JOIN gmp_audit1 gmp1 ON gmp.id = gmp1.gid
// -- INNER JOIN tblsaveans ts ON gmp.id = ts.qqid

// $query= "SELECT gmp.*, gmp2.ques, gmp1.date, gmp1.auditee,gmp1.audit,ts.quess,ts.cmnt,ts.ans,ts.cmnt
// FROM gmp_audit gmp
// INNER JOIN gmp_audit2 gmp2 ON gmp.id = gmp2.qid
// INNER JOIN gmp_audit1 gmp1 ON gmp.id = gmp1.gid
// INNER JOIN tblsaveans ts ON gmp.id = ts.qqid";

    // $sql = "SELECT * FROM `gmp_audit1`";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <tr>
        ...
        <td>
          <a href="audit-details-view.php?id=<?php echo $row["id"]; ?>" class="fa-solid fa-eye fs-5"></a>
        </td>
